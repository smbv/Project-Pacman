...
MapBuilder::MapBuilder(){
	...
	StrawberryC = ofColor(255,162,255);// remember to save rgb when psinting on the map
	CherryC = ofColor(128,128,255);
	MelonC = ofColor(103,237,187);
    ...
    for (int i = 0; i < mapImage.getWidth(); i++) {
        for (int j = 0; j < mapImage.getHeight(); j++) {
            ...
            if(currentPixel == boundBoundBlock){
                ...
                Player* PacMan = new Player(xPos,yPos,pixelMultiplier,pixelMultiplier, entityManager, mapImage);
			...
			else if(currentPixel == StrawberryC){ //creates the sprite into the map using rgb
			...
			else if(currentPixel == MelonC){
                Melon* melon = new Melon(xPos,yPos,pixelMultiplier,pixelMultiplier, pacmanSpriteSheet);
                mapInCreation->addEntity(melon);
            }
		}
    }
